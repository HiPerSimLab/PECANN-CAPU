...
def contour_prediction(model, cfg, trial, device):
    ...
    x_t_u_up = np.loadtxt( os.path.join(cfg.out_dir, f"{methodname}_{trial}_x_t_u_upred_f_fpred.dat") )
    ...
    f_star = x_t_u_up[:,4:5].reshape(test_dis[1], test_dis[0])
    f_pred = x_t_u_up[:,5:6].reshape(test_dis[1], test_dis[0])
    f_err  = np.abs(f_star - f_pred)
    
    varlist = ['u_star', 'u_pred', 'u_err', 'f_star', 'f_pred', 'f_err']
    for k, var in enumerate([u_star, u_pred, u_err, f_star, f_pred, f_err]):
...
def plot_update(trial, cfg):
    ...
    ax.plot(constr_s[:num_all,0], constr_s[:num_all,3], label=r'$\mathcal{C}_{F}$', linestyle=linestyles[3], color=colors[3],
    ...
    ax.plot(lambda_s[:num_all,0], lambda_s[:num_all,3], label=r'$\lambda_{F}$', linestyle=linestyles[2], color=colors[3],
            linewidth=2, marker=markers[2], markersize=5, markevery=num_all//10)
    ax.plot(mu_s[:num_all,0], mu_s[:num_all,3], label=r'$\mu_{F}$', linestyle=linestyles[2], color=colors[2],
            linewidth=2, marker=markers[2], markersize=5, markevery=num_all//10)
    ax.set_xlabel('Epochs', fontsize=14)
    ...
    ax.plot(l2_s[:num_all,0], l2_s[:num_all,1], label=r'$\mathcal{E}_r(\hat{u}-u)$', linestyle=linestyles[0], color=colors[3],
            linewidth=2)
    ax.plot(l2_s[:num_all,0], l2_s[:num_all,2], label=r'$\mathcal{E}_r(\hat{f}-f)$', linestyle=linestyles[3], color=colors[1],
            linewidth=2)
    ax.set_xlabel('Epochs', fontsize=14)
