...
def make_eval_fn(PDE_opt, flux_opt,
    ...
    def eval_objective_and_constraints(model, training=True):
        ...
        bc_loss = bc_res.square()#.mean() 
        ic_loss = ic_res.square()#.mean()
        q_loss  = q_res.square()#.mean()
        
        objective = pde_res.square().sum() # original PECANN formulation
        constr = torch.cat((bc_loss, ic_loss, q_loss), dim=0) 
