...
def contour_prediction(model, cfg, trial, device):
    ...
    x_t_u_up = np.loadtxt( os.path.join(cfg.out_dir, f"{methodname}_{trial}_x_t_u_upred.dat") )
    ...
    for k, var in enumerate([u_star, u_pred, u_err]):
        ...
        pimg=plt.pcolormesh(x, t, var, vmin=vmin, vmax=vmax, cmap=cmap_list[1], shading='gouraud')
