...
def sample_random_final_points(domain, n_data, dtype, device):
    lb, ub = domain[0], domain[1]
    soboleng = torch.quasirandom.SobolEngine(dimension=1, scramble=True)
    ...
    # top (vary x, fix t)
    x_top = draw(n_data) * (ub[0] - lb[0]) + lb[0]
    t_top = torch.full_like(x_top, ub[1])
...
def sampling_st(cfg: Config,
    ...
    if cfg.sampling == "uniform":
        ...
        x_data,t_data= sample_uniform_final_points(domain, cfg.dom_dis, dtype, device)
        ...
        x_data,t_data= sample_random_final_points(domain, cfg.n_data, dtype, device)
    else:
    ...
    x_data= x_data.requires_grad_(True);t_data= t_data.requires_grad_(True)
    return x_dom, t_dom, x_bc, t_bc, x_ic, t_ic, x_data, t_data
