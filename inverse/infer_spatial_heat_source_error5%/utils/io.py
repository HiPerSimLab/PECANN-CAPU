...
def save_st_points(x_dom, t_dom, x_bc, t_bc, x_ic, t_ic, x_data,t_data, out_dir, trial):
    ...
    data_fc  = torch.cat((x_data,t_data),  dim=1).detach().cpu().numpy()
    ...
    np.savetxt(os.path.join(out_dir, f"{trial}_data.dat"),  data_fc,  fmt="%.6e", delimiter=" ") 
