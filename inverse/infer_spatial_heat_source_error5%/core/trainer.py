...
from physics import PDE_opt, boundary_opt, initial_opt, data_opt
from data import sampling_st
from viz import contour_prediction, plot_update, scatter_st_collocation
from models import doubleforward_FC_Net, xavier_init
...
def make_eval_fn(PDE_opt, n,
                boundary_opt, initial_opt, data_opt,
                x_dom,t_dom, x_bc,t_bc, x_ic,t_ic, x_data,t_data,
                ):

    # eval_fn(model) -> objective, constr
    def eval_objective_and_constraints(model, training=True):
        data_res = data_opt(model, x_data,t_data, n)
        ...
        avg_pde_loss = pde_res.square().mean()
        avg_bc_loss  = bc_res.square().mean() 
        avg_ic_loss  = ic_res.square().mean()
        
        objective = data_res.square().mean() # avg_data_loss
        constr = torch.stack([avg_bc_loss, avg_ic_loss, avg_pde_loss]).unsqueeze(-1)
...
class Trainer:
    ...
    def build_model(self) -> dict:
        ...
        model = doubleforward_FC_Net(layers, mean_dom_st, stdev_dom_st,
    ...
    def run(self):
        ...
        for trial in range(1, self.cfg.trials+1):
            ...
            # Sampling
            x_dom, t_dom, x_bc, t_bc, x_ic, t_ic, x_data,t_data = sampling_st(self.cfg)
            save_st_points(x_dom,t_dom, x_bc,t_bc, x_ic, t_ic, x_data,t_data, self.cfg.out_dir, trial)
            ...
            for epoch in pbar:
                ...
                if self.cfg.print_to_console and (epoch % self.cfg.disp == 0):
                    ...
                    l2_u, l2_f, linf_u, linf_f = evaluate_norms_write(model, self.cfg, trial, self.device, write=False)
                    l2_buf.append([epoch, l2_u, l2_f])
                    linf_buf.append([epoch, linf_u, linf_f])
                    
                if epoch % self.cfg.disp2 == 0:
                    #l2_u, l2_f, linf_u, linf_f = evaluate_norms_write(model, self.cfg, trial, self.device, write=True)
                    print('rela. l2 norms of u and f = %.3e, %.3e'%(l2_u, l2_f))
